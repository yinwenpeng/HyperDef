...
def transfer_wordlist_2_idlist_with_maxlen_fixed_vocab(token_list, vocab_map, maxlen):
    '''
...
def write_predictions_to_analysis(test_size, pred_labels,probs):
    root="/save/wenpeng/datasets/wordnet-related/word-hyper-def-PairPerLine.txt"
    readfile=open(root, 'r')
    lines = []
    for line in readfile:
        lines.append(line.strip())
    readfile.close()
    writefile =open("/save/wenpeng/datasets/wordnet-related/word-hyper-def-PairPerLine_testanalysis.txt",'w')
    writepart=lines[-test_size/2:]

    for i in range(test_size/2):
        parts=writepart[i].strip().split('\t')
        writefile.write('1\t'+str(pred_labels[i*2])+'\t'+str(probs[i*2])+'\n')
        writefile.write(writepart[i].strip()+'\n')
        writefile.write('0\t'+str(pred_labels[i*2+1])+'\t'+str(probs[i*2+1])+'\n')
        writefile.write(parts[2]+'\t'+parts[3]+'\t'+parts[0]+'\t'+parts[1]+'\n\n')
    writefile.close()
    print 'write analusis over'


def load_EVAlution_hyper_vs_all(maxlen, word2id):
    root="/save/wenpeng/datasets/EVALution/EVALution.test.definitions.hyper.vs.all"
    all_sentences_l=[]
    all_masks_l=[]
    all_sentences_r=[]
    all_masks_r=[]
    ...
    for line in readfile:
        ...
        if len(parts)==5:
            size+=1
            sentence_wordlist_l=parts[2].strip().lower().split()
            sentence_wordlist_r=parts[4].strip().lower().split()
            ...
            all_sentences_l.append(sent_idlist_l)
            all_sentences_r.append(sent_idlist_r)
            all_masks_l.append(sent_masklist_l)
            all_masks_r.append(sent_masklist_r)
...
def store_word2id(word2id, filename):
    with open(filename, 'wb') as f:
        pickle.dump(word2id, f, pickle.HIGHEST_PROTOCOL)
    print 'word2id stored over'

def load_word2id(filename):
    with open(filename, 'rb') as f:
        return pickle.load(f)

def parse_individual_termPair(term1, term2, def1, def2, maxlen, word2id, wordlen=4):
    # root="/save/wenpeng/datasets/EVALution/EVALution.test.definitions.hyper.vs.all"
    all_sentences_l=[]
    all_masks_l=[]
    all_sentences_r=[]
    ...
    sentence_wordlist_l=def1.strip().lower().split()
    sentence_wordlist_r=def2.strip().lower().split()


    sent_idlist_l, sent_masklist_l=transfer_wordlist_2_idlist_with_maxlen_fixed_vocab(sentence_wordlist_l, word2id, maxlen)
    sent_idlist_r, sent_masklist_r=transfer_wordlist_2_idlist_with_maxlen_fixed_vocab(sentence_wordlist_r, word2id, maxlen)

    word1_wordlist = term1.strip().lower().split()
    word2_wordlist = term2.strip().lower().split()
    word1_ids, word1_mask=transfer_wordlist_2_idlist_with_maxlen_fixed_vocab(word1_wordlist, word2id, wordlen)
    word2_ids, word2_mask=transfer_wordlist_2_idlist_with_maxlen_fixed_vocab(word2_wordlist, word2id, wordlen)


    if len(set(word2_wordlist) & set(sentence_wordlist_l)) > 0:
        all_extra.append(1.0)
    else:
        all_extra.append(0.0)
    ...
    all_masks_l.append(sent_masklist_l)
    all_masks_r.append(sent_masklist_r)
    all_word1.append(word1_ids)
    all_word2.append(word2_ids)
    all_word1_mask.append(word1_mask)
    all_word2_mask.append(word2_mask)

    print 'Term pair parsed successfully '
    return all_sentences_l, all_masks_l, all_sentences_r, all_masks_r,all_word1,all_word2,all_word1_mask,all_word2_mask, all_extra,word2id


def load_EVAlution_hyper_vs_all_with_words(maxlen, word2id, wordlen=4):
    root="/save/wenpeng/datasets/EVALution/EVALution.test.definitions.hyper.vs.all"
    all_sentences_l=[]
    all_masks_l=[]
    all_sentences_r=[]
    ...
    for line in readfile:
        ...
        if len(parts)==5:
            ...
            word1_wordlist = parts[1].strip().lower().split()
            word2_wordlist = parts[3].strip().lower().split()
            word1_ids, word1_mask=transfer_wordlist_2_idlist_with_maxlen(word1_wordlist, word2id, wordlen)
            word2_ids, word2_mask=transfer_wordlist_2_idlist_with_maxlen(word2_wordlist, word2id, wordlen)

            extra_feature = hypernymy_detection_extra_features(parts[2], parts[1], parts[3])
            all_extra.append(extra_feature)
    ...
    print '\t\t\t size:', size, 'pairs'
    print 'dataset loaded over, totally ', len(word2id), 'words, max sen len:',   max_sen_len
    return all_sentences_l, all_masks_l, all_sentences_r, all_masks_r,all_word1,all_word2,all_word1_mask,all_word2_mask,all_labels, all_extra,word2id

def load_task_hyper_vs_all_with_words(root, maxlen, word2id, wordlen=4):
    # root="/save/wenpeng/datasets/EVALution/EVALution.test.definitions.hyper.vs.all"
    all_sentences_l=[]
    all_masks_l=[]
    all_sentences_r=[]
...
def load_EVAlution_hyper_vs_all_with_allDefComb(maxlen, word2id, wordlen=4):
    #seems have bugs
    root="/save/wenpeng/datasets/EVALution/EVALution.test.all.def.combinations.binary"
    all_sentences_l=[]
    all_masks_l=[]
    all_sentences_r=[]
    all_masks_r=[]
    ...
    for line in readfile:
        ...
        if len(parts)==5:
            ...
            term_pair = (parts[1].strip(), parts[3].strip())
            if term_pair != ini_term_pair and size>1:
                #load
                ini_term_pair = term_pair
    ...
    group_size_list.append(group_size)
    # print len(group_size_list), numpy.sum(numpy.asarray(group_size_list)), len(all_labels)
    # exit(0)
    print '\t\t\t size:', size, 'pairs'
    print 'dataset loaded over, totally ', len(word2id), 'words, max sen len:',   max_sen_len
    return all_sentences_l, all_masks_l, all_sentences_r, all_masks_r,all_word1,all_word2,all_word1_mask,all_word2_mask,all_labels, all_extra,word2id,group_size_list

def load_task_hyper_vs_all_with_allDefComb(root, maxlen, word2id, wordlen=4):
    # root="/save/wenpeng/datasets/EVALution/EVALution.test.all.def.combinations.binary"
    all_sentences_l=[]
    all_masks_l=[]
    all_sentences_r=[]
...
def load_HypeNet_testset(path, keyword,maxlen, word2id, wordlen=4):
    root=path+keyword+".as.input.txt"
    all_sentences_l=[]
    all_masks_l=[]
    all_sentences_r=[]
    all_masks_r=[]
    ...
    readfile=codecs.open(root, 'r' ,'utf-8')
    size = 0
    pos_size=0
    for line in readfile:

        parts=line.strip().lower().split('\t') #label, term1, def1, term2, def2
        if len(parts)==5:
            size+=1
            sentence_wordlist_l=parts[2].strip().lower().split()
            sentence_wordlist_r=parts[4].strip().lower().split()
            ...
            word1_wordlist = parts[1].strip().split()
            word2_wordlist = parts[3].strip().split()
            word1_ids, word1_mask=transfer_wordlist_2_idlist_with_maxlen(word1_wordlist, word2id, wordlen)
            word2_ids, word2_mask=transfer_wordlist_2_idlist_with_maxlen(word2_wordlist, word2id, wordlen)
            ...
            # posi_word1 = parts[2].strip().lower().find(parts[1].strip().lower())
            # posi_word2 = parts[2].strip().lower().find(parts[3].strip().lower())
            # if posi_word2>posi_word1:
            #     if len(parts[2].strip().lower()[posi_word1:posi_word2].split())<5:
            #         all_extra.append(1.0)
            #     else:
            #         all_extra.append(0.0)
            # else:
            ...
            # if parts[3].strip().lower() in set(sentence_wordlist_l):
            #     all_extra.append(1.0)
            #
            # else:
            #     all_extra.append(0.0)
    ...
    print '\t\t\t size:', size, 'pairs, pos size: ', pos_size, ' neg size: ', size-pos_size
    print 'dataset loaded over, totally ', len(word2id), 'words, max sen len:',   max_sen_len
    return all_sentences_l, all_masks_l, all_sentences_r, all_masks_r,all_word1,all_word2,all_word1_mask,all_word2_mask,all_labels, all_extra,word2id


def load_wordnet_hyper_dataset(maxlen=40):
    root="/save/wenpeng/datasets/wordnet-related/word-hyper-def-PairPerLine.txt"
    word2id={}  # store vocabulary, each word map to a id
    all_sentences_l=[]
    all_masks_l=[]
    ...
    for line in readfile:
        ...
        if len(parts)==4:
            ...
            sentence_wordlist_r=parts[3].strip().lower().split()
            ...
            all_sentences_l.append(sent_idlist_r)
            all_sentences_r.append(sent_idlist_l)
            all_masks_l.append(sent_masklist_r)
            all_masks_r.append(sent_masklist_l)
            all_labels.append(0)
...
def wordStr_to_wordId(word2id, wordstr):
    id = word2id.get(wordstr)
    if id is None:
        id=len(word2id)+1  # id of true words starts from 1, leaving 0 to "pad id"
        word2id[wordstr]=id
    return id

def load_TaxRel_DuangDo(maxlen=40, wordlen=2, word2id=None, filname =''):
    root="/save/wenpeng/datasets/TaxonomicRelationData/DataII/"+filname+'.in.def.txt'
    # root="/save/wenpeng/datasets/wordnet-related/word-hyper-def-PairPerLine-binary.txt"
    if word2id is None:
        word2id={}  # store vocabulary, each word map to a id
    all_sentences_l=[]
    all_masks_l=[]
    ...
    for line in readfile:
        ...
        if len(parts)==5 and parts[0] in set(['0','1','2','3']):
            size+=1
            sentence_wordlist_l=parts[2].strip().split()
            sentence_wordlist_r=parts[4].strip().split()
            ...
            word1_wordlist = parts[1].split()
            word1_ids, word1_mask =transfer_wordlist_2_idlist_with_maxlen(word1_wordlist, word2id, wordlen)
            word2_wordlist = parts[3].split()
            word2_ids, word2_mask =transfer_wordlist_2_idlist_with_maxlen(word2_wordlist, word2id, wordlen)

            extra_feature = hypernymy_detection_extra_features(parts[2], ' '.join(word1_wordlist), ' '.join(word2_wordlist))
            all_extra.append(extra_feature)
            # if ' '.join(word2_wordlist) in set(sentence_wordlist_l):
            #     all_extra.append(1.0)
            # else:
            #     all_extra.append(0.0)
    ...
    # print '\t\t\t size:', size, 'pairs, pos size: ', pos_size, ' neg_size: ', neg_size
    print 'dataset loaded over, totally ', len(word2id), 'words, max sen len:',   max_sen_len
    return all_sentences_l, all_masks_l, all_sentences_r, all_masks_r,all_word1,all_word2, all_word1_mask,all_word2_mask,all_labels, all_extra,word2id

def statistics():
    "first, vocab set of wn_train"
    root="/save/wenpeng/datasets/wordnet-related/word-hyper-vs-all-PairPerLine.txt"
    readfile=open(root, 'r')
    word_set = set()
    pair_set = set()
    line_co=0
    for line in readfile:
        ...
        parts=line.strip().lower().split('\t') #lowercase all tokens, as we guess this is not important for sentiment task
        if len(parts)==6:
            word1_wordlist = parts[1][:parts[1].find('.')].split('_')

            word2_wordlist = parts[3][:parts[3].find('.')].split('_')
            for word in word1_wordlist:
                word_set.add(word)
            for word in word2_wordlist:
                word_set.add(word)
            pair_set.add((' '.join(word1_wordlist), ' '.join(word2_wordlist)))
            line_co+=1
            # if line_co % 1000==0:
            #     print 'line_co ', line_co
    print 'word_set size:', len(word_set)
    print 'pair_set size:', len(pair_set)
    readfile.close()
    "EVALution"
    EVALutionroot="/save/wenpeng/datasets/UnsupervisedHypernymy/EVALution.test"
    EVALution_readfile=open(EVALutionroot, 'r')
    EVALution_word_set = set()
    EVALution_pair_set = set()
    line_co=0
    for line in EVALution_readfile:

        parts=line.strip().lower().split('\t') #lowercase all tokens, as we guess this is not important for sentiment task

        word1 = parts[0][:-2]
        word2 = parts[1][:-2]
        EVALution_word_set.add(word1)
        EVALution_word_set.add(word2)
        EVALution_pair_set.add((word1, word2))
        line_co+=1
        # if line_co % 1000==0:
        #     print 'line_co ', line_co
    print 'EVALution_word_set size:', len(EVALution_word_set)
    print 'EVALution_pair_set size:', len(EVALution_pair_set)
    print "EVALution_word_set overlap size:", len(word_set&EVALution_word_set)
    print "EVALution_pair_set overlap size:", len(pair_set&EVALution_pair_set), 1.0-len(pair_set&EVALution_pair_set)*1.0/len(EVALution_pair_set)

    "BLESS"
    blessroot= '/save/wenpeng/datasets/UnsupervisedHypernymy/BLESS.test'
    bless_readfile=open(blessroot, 'r')
    bless_word_set = set()
    bless_pair_set = set()
    # line_co=0
    for line in bless_readfile:

        parts=line.strip().lower().split('\t') #lowercase all tokens, as we guess this is not important for sentiment task

        word1 = parts[0][:-2]
        word2 = parts[1][:-2]
        bless_word_set.add(word1)
        bless_word_set.add(word2)
        bless_pair_set.add((word1, word2))
        # line_co+=1
        # if line_co % 1000==0:
        #     print 'line_co ', line_co
    print 'bless_word_set size:', len(bless_word_set)
    print 'bless_pair_set size:', len(bless_pair_set)
    print "bless_word_set overlap size:", len(word_set&bless_word_set)
    print "bless_pair_set overlap size:", len(pair_set&bless_pair_set), 1.0-len(pair_set&bless_pair_set)*1.0/len(bless_pair_set)

    "LenciBenotto"
    LenciBenottoroot= '/save/wenpeng/datasets/UnsupervisedHypernymy/LenciBenotto.test'
    LenciBenotto_readfile=open(LenciBenottoroot, 'r')
    LenciBenotto_word_set = set()
    LenciBenotto_pair_set = set()
    # line_co=0
    for line in LenciBenotto_readfile:

        parts=line.strip().lower().split('\t') #lowercase all tokens, as we guess this is not important for sentiment task

        word1 = parts[0][:-2]
        word2 = parts[1][:-2]
        LenciBenotto_word_set.add(word1)
        LenciBenotto_word_set.add(word2)
        LenciBenotto_pair_set.add((word1, word2))
        # line_co+=1
        # if line_co % 1000==0:
        #     print 'line_co ', line_co
    print 'LenciBenotto_word_set size:', len(LenciBenotto_word_set)
    print 'LenciBenotto_pair_set size:', len(LenciBenotto_pair_set)
    print "LenciBenotto_word_set overlap size:", len(word_set&LenciBenotto_word_set)
    print "LenciBenotto_pair_set overlap size:", len(pair_set&LenciBenotto_pair_set), 1.0-len(pair_set&LenciBenotto_pair_set)*1.0/len(LenciBenotto_pair_set)

    "Weeds"
    Weedsroot= '/save/wenpeng/datasets/UnsupervisedHypernymy/Weeds.test'
    Weeds_readfile=open(Weedsroot, 'r')
    Weeds_word_set = set()
    Weeds_pair_set = set()
    # line_co=0
    for line in Weeds_readfile:

        parts=line.strip().lower().split('\t') #lowercase all tokens, as we guess this is not important for sentiment task

        word1 = parts[0][:-2]
        word2 = parts[1][:-2]
        Weeds_word_set.add(word1)
        Weeds_word_set.add(word2)
        Weeds_pair_set.add((word1, word2))
        # line_co+=1
        # if line_co % 1000==0:
        #     print 'line_co ', line_co
    print 'Weeds_word_set size:', len(Weeds_word_set)
    print 'Weeds_pair_set size:', len(Weeds_pair_set)
    print "Weeds_word_set overlap size:", len(word_set&Weeds_word_set)
    print "Weeds_pair_set overlap size:", len(pair_set&Weeds_pair_set), 1.0-len(pair_set&Weeds_pair_set)*1.0/len(Weeds_pair_set)

    "HypeNet lexical"
    HypeNet_lex_root= '/save/wenpeng/datasets/HypeNet/dataset_lex/test.tsv'
    HypeNet_lex_readfile=open(HypeNet_lex_root, 'r')
    HypeNet_lex_word_set = set()
    HypeNet_lex_pair_set = set()
    # line_co=0
    for line in HypeNet_lex_readfile:

        parts=line.strip().lower().split('\t') #lowercase all tokens, as we guess this is not important for sentiment task

        word1 = parts[0].strip()
        word2 = parts[1].strip()
        HypeNet_lex_word_set.add(word1)
        HypeNet_lex_word_set.add(word2)
        HypeNet_lex_pair_set.add((word1, word2))
        # line_co+=1
        # if line_co % 1000==0:
        #     print 'line_co ', line_co
    print 'HypeNet_lex_word_set size:', len(HypeNet_lex_word_set)
    print 'HypeNet_lex_pair_set size:', len(HypeNet_lex_pair_set)
    print "HypeNet_lex_word_set overlap size:", len(word_set&HypeNet_lex_word_set)
    print "HypeNet_lex_pair_set overlap size:", len(pair_set&HypeNet_lex_pair_set), 1.0-len(pair_set&HypeNet_lex_pair_set)*1.0/len(HypeNet_lex_pair_set)

    "HypeNet random"
    HypeNet_rnd_root= '/save/wenpeng/datasets/HypeNet/dataset_rnd/test.tsv'
    HypeNet_rnd_readfile=open(HypeNet_rnd_root, 'r')
    HypeNet_rnd_word_set = set()
    HypeNet_rnd_pair_set = set()
    # line_co=0
    for line in HypeNet_rnd_readfile:

        parts=line.strip().lower().split('\t') #lowercase all tokens, as we guess this is not important for sentiment task

        word1 = parts[0].strip()
        word2 = parts[1].strip()
        HypeNet_rnd_word_set.add(word1)
        HypeNet_rnd_word_set.add(word2)
        HypeNet_rnd_pair_set.add((word1, word2))
        # line_co+=1
        # if line_co % 1000==0:
        #     print 'line_co ', line_co
    print 'HypeNet_rnd_word_set size:', len(HypeNet_rnd_word_set)
    print 'HypeNet_rnd_pair_set size:', len(HypeNet_rnd_pair_set)
    print "HypeNet_rnd_word_set overlap size:", len(word_set&HypeNet_rnd_word_set)
    print "HypeNet_rnd_pair_set overlap size:", len(pair_set&HypeNet_rnd_pair_set), 1.0-len(pair_set&HypeNet_rnd_pair_set)*1.0/len(HypeNet_rnd_pair_set)


def HypeNet_statistics():
    "HypeNet train"
    train_root= '/save/wenpeng/datasets/HypeNet/dataset_lex/train.tsv'
    train_readfile=open(train_root, 'r')
    train_word_set = set()
    train_pair_set = set()
    # line_co=0
    for line in train_readfile:

        parts=line.strip().lower().split('\t') #lowercase all tokens, as we guess this is not important for sentiment task

        word1 = parts[0].strip()
        word2 = parts[1].strip()
        train_word_set.add(word1)
        train_word_set.add(word2)
        train_pair_set.add((word1, word2))
        # line_co+=1
        # if line_co % 1000==0:
        #     print 'line_co ', line_co
    print 'train_word_set size:', len(train_word_set)
    print 'train_pair_set size:', len(train_pair_set)

    "HypeNet test"
    HypeNet_rnd_root= '/save/wenpeng/datasets/HypeNet/dataset_lex/test.tsv'
    HypeNet_rnd_readfile=open(HypeNet_rnd_root, 'r')
    HypeNet_rnd_word_set = set()
    HypeNet_rnd_pair_set = set()
    # line_co=0
    ...
    print 'HypeNet_rnd_word_set size:', len(HypeNet_rnd_word_set)
    print 'HypeNet_rnd_pair_set size:', len(HypeNet_rnd_pair_set)
    print "HypeNet_rnd_word_set overlap size:", len(train_word_set&HypeNet_rnd_word_set)
    print "HypeNet_rnd_pair_set overlap size:", len(train_pair_set&HypeNet_rnd_pair_set), 1.0-len(train_pair_set&HypeNet_rnd_pair_set)*1.0/len(HypeNet_rnd_pair_set)


def load_wordnet_hyper_vs_all_with_words(maxlen=40, wordlen=4):
    root="/save/wenpeng/datasets/wordnet-related/word-hyper-vs-all-PairPerLine.txt"
    # root="/save/wenpeng/datasets/wordnet-related/word-hyper-def-PairPerLine-binary.txt"
...
def hypernymy_detection_extra_features(def1_str, word1_str, word2_str):
    refined_def1_str = def1_str.strip().lower()
    refined_word1_str = word1_str.strip().lower()
    refined_word2_str = word2_str.strip().lower()

    posi_word1 = refined_def1_str.find(refined_word1_str)
    posi_word2 = refined_def1_str.find(refined_word2_str)
    if posi_word2>posi_word1:
        if len(refined_def1_str[posi_word1:posi_word2].split())<5:
            return 1.0
        else:
            return 0.0
    else:
...
def load_hyper_meta_dataset(maxlen=40, wordlen=4):
    # root="/save/wenpeng/datasets/wordnet-related/word-hyper-vs-all-PairPerLine.txt"
    root="/save/wenpeng/datasets/wordnet-related/meta_data_hyper_or_not.txt"
...
def load_wordnet_hyper_dataset_with_binary(maxlen=40, wordlen=4):
    # root="/save/wenpeng/datasets/wordnet-related/word-hyper-vs-all-PairPerLine.txt"
    root="/save/wenpeng/datasets/wordnet-related/word-hyper-def-PairPerLine-binary.txt"
...
def load_HypeNet_trainset(maxlen=40, wordlen=4):
    # root="/save/wenpeng/datasets/wordnet-related/word-hyper-vs-all-PairPerLine.txt"
    root="/save/wenpeng/datasets/HypeNet/dataset_lex/train.as.input.txt"
...
def load_wordnet_hyper_dataset_with_words(maxlen=40, wordlen=4):
    '''
    in this function, we load hyperdata: hyper and switched hypo is pos, when loading, swithcing again as negative
    '''
    root="/save/wenpeng/datasets/wordnet-related/word-hyper-def-PairPerLine.txt"
    word2id={}  # store vocabulary, each word map to a id
    all_sentences_l=[]
    ...
    for line in readfile:
        ...
        if len(parts)==4:
            ...
            word1_wordlist = parts[0][:parts[0].find('.')].split('_')
            word1_ids, word1_mask =transfer_wordlist_2_idlist_with_maxlen(word1_wordlist, word2id, wordlen)
            word2_wordlist = parts[2][:parts[2].find('.')].split('_')
            word2_ids, word2_mask =transfer_wordlist_2_idlist_with_maxlen(word2_wordlist, word2id, wordlen)
            ...
            all_masks_l.append(sent_masklist_r)
            all_masks_r.append(sent_masklist_l)
            all_word1.append(word2_ids)
            all_word2.append(word1_ids)
            all_word1_mask.append(word2_mask)
            all_word2_mask.append(word1_mask)
            all_labels.append(0)

    print '\t\t\t size:', size, 'pairs'
    print 'dataset loaded over, totally ', len(word2id), 'words, max sen len:',   max_sen_len
    return all_sentences_l, all_masks_l, all_sentences_r, all_masks_r,all_word1,all_word2, all_word1_mask,all_word2_mask,all_labels, word2id
...
def load_SNLI_testset(maxlen, word2id):
...
def load_ACE05_dataset(maxlen, word2id):
    root="/mounts/data/proj/wenpeng/Dataset/ACE05/"
    files=['ACE05-DocStatements-tokenized.txt']
    # word2id={}  # store vocabulary, each word map to a id
    sents_l=[]
    sents_masks_l=[]
    sents_r=[]
