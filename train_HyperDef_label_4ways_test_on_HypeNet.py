...
from load_data import store_word2id, load_wordnet_hyper_vs_all_with_words, write_predictions_to_analysis,load_wordnet_hyper_dataset_with_binary,load_word2vec, load_word2vec_to_init, load_HypeNet_testset
from common_functions import apk, Conv_for_Pair,dropout_layer, store_model_to_file, elementwise_is_two,Conv_with_Mask_with_Gate, Conv_with_Mask, create_conv_para, L2norm_paraList, ABCNN, create_ensemble_para, cosine_matrix1_matrix2_rowwise, Diversify_Reg, Gradient_Cost_Para, GRU_Batch_Tensor_Input_with_Mask, f1_score_binary
'''
lex:
current test_f1: 0.607329842932 0.658093797277 0.563836681789
current test_f1: 0.614637109019 0.765506807867 0.513444951801
current test_f1: 0.629667003027 0.708018154312 0.566929133858
current test_f1: 0.636151019044 0.720121028744 0.569718731299
current test_f1: 0.636668905306 0.717095310136 0.572463768116

rnd:
current test_f1: 0.529485570891 0.600797266515 0.473306415433
current test_f1: 0.530053959091 0.601366742597 0.47386134171
'''

def evaluate_lenet5(learning_rate=0.01, n_epochs=4, L2_weight=1e-5, extra_size=4, emb_size=300, batch_size=100, filter_size=[3,3], maxSentLen=20, hidden_size=[300,300], max_term_len=4, p_mode = 'conc'):
    ...
    root = '/save/wenpeng/datasets/HypeNet/dataset_rnd/'
    # root = '/save/wenpeng/datasets/HypeNet/dataset_rnd/'
    ...
    # all_sentences_l, all_masks_l, all_sentences_r, all_masks_r, all_word1,all_word2,all_word1_mask,all_word2_mask,all_labels, all_extra, word2id  =load_wordnet_hyper_dataset_with_binary(maxlen=maxSentLen, wordlen=max_term_len)  #minlen, include one label, at least one word in the sentence
    test_sents_l, test_masks_l, test_sents_r, test_masks_r, test_word1,test_word2,test_word1_mask,test_word2_mask,test_labels, test_extra, word2id = load_HypeNet_testset(root,'test',maxSentLen, word2id, wordlen=max_term_len)
    ...
    test_model = theano.function([sents_ids_l, sents_mask_l, sents_ids_r, sents_mask_r, word1_ids,word2_ids,word1_mask,word2_mask,extra,labels], layer_LR.y_pred, allow_input_downcast=True, on_unused_input='ignore')
        ...
        for batch_id in train_batch_start: #for each batch
            ...
            if iter%100==0:
                ...
                for idd, test_batch_id in enumerate(test_batch_start): # for each test batch
                    ...
                    # probs+=list(prob_i)
                if n_test_remain !=0:
                    pred_labels = pred_labels[:(len(test_batch_start)-1)*batch_size]+pred_labels[-n_test_remain:]
                assert len(test_labels) == len(pred_labels)
                # test_acc=1.0-error_sum/(len(test_batch_start))

                test_f1, recall, precision = f1_score_binary(test_labels, pred_labels)


                if test_f1 > max_f1:
                    max_f1=test_f1
                    store_model_to_file(root+'Best_para_'+str(max_f1), params)
                print '\t\tcurrent test_f1:', test_f1,recall, precision, ' ; ','\t\tmax_f1: ', max_f1
