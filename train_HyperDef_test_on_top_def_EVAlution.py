...
def evaluate_lenet5(learning_rate=0.02, n_epochs=4, L2_weight=1e-5, extra_size=4, emb_size=300, batch_size=100, filter_size=[3,3], maxSentLen=40, hidden_size=[300,300], max_term_len=4, p_mode = 'conc'):
    ...
    test_sents_l, test_masks_l, test_sents_r, test_masks_r, test_word1,test_word2,test_word1_mask,test_word2_mask,test_labels, test_extra, word2id = load_task_hyper_vs_all_with_words('/save/wenpeng/datasets/EVALution/EVALution.test.definitions.hyper.vs.all',maxSentLen, word2id, wordlen=max_term_len)
    store_word2id(word2id, root_dic+'EVALution_word2id.pkl')
    # exit(0)
    ...
    store_model_to_file(root_dic+'EVALution_best_para_init_embeddings', [init_embeddings])
        ...
        for batch_id in train_batch_start: #for each batch
            ...
            if iter%100==0:
                ...
                if test_ap > max_ap_test:
                    max_ap_test=test_ap
                    store_model_to_file(root_dic+'EVALution_best_para_'+str(max_ap_test), params)
