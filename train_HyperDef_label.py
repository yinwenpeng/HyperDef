...
from load_data import load_SNLI_dataset, write_predictions_to_analysis,load_wordnet_hyper_vs_all_with_words,load_word2vec, load_word2vec_to_init, load_EVAlution_hyper_vs_all_with_words
...
def evaluate_lenet5(learning_rate=0.02, n_epochs=4, L2_weight=1e-5, extra_size=4, emb_size=300, batch_size=20, filter_size=[3,3], maxSentLen=40, hidden_size=[300,300], max_term_len=4):
    ...
    all_sentences_l, all_masks_l, all_sentences_r, all_masks_r, all_word1,all_word2,all_word1_mask,all_word2_mask,all_labels, word2id  =load_wordnet_hyper_vs_all_with_words(maxlen=maxSentLen, wordlen=max_term_len)  #minlen, include one label, at least one word in the sentence
    # test_sents_l, test_masks_l, test_sents_r, test_masks_r, test_labels, word2id  =load_ACE05_dataset(maxSentLen, word2id)
    test_sents_l, test_masks_l, test_sents_r, test_masks_r, test_word1,test_word2,test_word1_mask,test_word2_mask,test_labels, word2id = load_EVAlution_hyper_vs_all_with_words(maxSentLen, word2id, wordlen=max_term_len)
    ...
    embed_word1 = init_embeddings[word1_ids.flatten()].reshape((batch_size,max_term_len, emb_size))
    embed_word2 = init_embeddings[word2_ids.flatten()].reshape((batch_size,max_term_len, emb_size))
    ...
    LR_input = T.concatenate([attentive_sent_embeddings_l,attentive_sent_embeddings_r,attentive_sent_embeddings_l*attentive_sent_embeddings_r,attentive_sent_embeddings_l-attentive_sent_embeddings_r,word1_embedding,word2_embedding,word1_embedding*word2_embedding],axis=1)
    ...
    train_model = theano.function([sents_ids_l, sents_mask_l, sents_ids_r, sents_mask_r, word1_ids,word2_ids,word1_mask,word2_mask,labels], cost, updates=updates, allow_input_downcast=True, on_unused_input='ignore')
    test_model = theano.function([sents_ids_l, sents_mask_l, sents_ids_r, sents_mask_r, word1_ids,word2_ids,word1_mask,word2_mask,labels], [layer_LR.errors(labels),layer_LR.y_pred,layer_LR.prop_for_posi], allow_input_downcast=True, on_unused_input='ignore')
