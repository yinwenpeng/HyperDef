...
from load_data import store_word2id, load_SNLI_dataset, write_predictions_to_analysis,load_hyper_meta_dataset,load_word2vec, load_word2vec_to_init, load_HypeNet_testset
...
def evaluate_lenet5(learning_rate=0.02, n_epochs=4, L2_weight=1e-5, extra_size=4, emb_size=300, batch_size=100, filter_size=[3,3], maxSentLen=40, hidden_size=[300,300], max_term_len=4, p_mode = 'conc'):
    ...
    all_sentences_l, all_masks_l, all_sentences_r, all_masks_r, all_word1,all_word2,all_word1_mask,all_word2_mask,all_labels, all_extra, word2id  =load_hyper_meta_dataset(maxlen=maxSentLen, wordlen=max_term_len)  #minlen, include one label, at least one word in the sentence
    # test_sents_l, test_masks_l, test_sents_r, test_masks_r, test_labels, word2id  =load_ACE05_dataset(maxSentLen, word2id)
    test_sents_l, test_masks_l, test_sents_r, test_masks_r, test_word1,test_word2,test_word1_mask,test_word2_mask,test_labels, test_extra, word2id = load_HypeNet_testset(maxSentLen, word2id, wordlen=max_term_len)
    # store_word2id(word2id, '/save/wenpeng/datasets/HypeNet/HyperDef_label_meta_best_para_word2id.pkl')
    # exit(0)
    ...
    while epoch < n_epochs:
        ...
        for batch_id in train_batch_start: #for each batch
            ...
            if iter%100==0:
                ...
                # test_extra_int = [int(x) for x in test_extra]
                # ensemble_label = np.asarray(test_extra_int)+np.asarray(pred_labels)
                # ensemble_label[ensemble_label>0]=1
                # pred_labels_ensemble = list(ensemble_label)

                test_f1 = f1_score_binary(test_labels, pred_labels)
                # test_f1_ensemble = f1_score_binary(test_labels, pred_labels_ensemble)


                if test_f1[0] > max_f1:
                    max_f1=test_f1[0]
                    store_model_to_file('/save/wenpeng/datasets/HypeNet/HyperDef_label_meta_best_para_'+str(max_f1), params)

                # if test_f1_ensemble[0] > max_f1_ensemble:
                #     max_f1_ensemble=test_f1_ensemble[0]

                print '\t\tcurrent test_f1:', test_f1,' ; ','\t\tmax_f1: ', max_f1
                # print '\t\tcurrent test_f1_ensemble:', test_f1_ensemble,' ; ','\t\tmax_f1_en: ', max_f1_ensemble
