...
from load_data import load_SNLI_testset, write_predictions_to_analysis,load_wordnet_hyper_dataset,load_word2vec, load_word2vec_to_init, load_EVAlution_hyper_vs_all
from common_functions import f1_score_binary, Conv_for_Pair,dropout_layer, store_model_to_file, elementwise_is_two,Conv_with_Mask_with_Gate, Conv_with_Mask, create_conv_para, L2norm_paraList, ABCNN, create_ensemble_para, cosine_matrix1_matrix2_rowwise, Diversify_Reg, Gradient_Cost_Para, GRU_Batch_Tensor_Input_with_Mask, create_LSTM_para
'''
1, does nt word well, only .53 f1
'''
...
def evaluate_lenet5(learning_rate=0.02, n_epochs=4, L2_weight=0.0000001, extra_size=4, emb_size=300, batch_size=50, filter_size=[3,3], maxSentLen=40, hidden_size=[300,300]):
    ...
    test_sents_l, test_masks_l, test_sents_r, test_masks_r, test_labels, word2id  =load_SNLI_testset(maxSentLen,word2id)
    ...
    test_sents_l=np.asarray(test_sents_l[0], dtype='int32')
    ...
    test_masks_l=np.asarray(test_masks_l[0], dtype=theano.config.floatX)
    ...
    test_sents_r=np.asarray(test_sents_r[0], dtype='int32')
    ...
    test_masks_r=np.asarray(test_masks_r[0], dtype=theano.config.floatX)
    ...
    test_labels_store=np.asarray(test_labels[0], dtype='int32')
    ...
    test_model = theano.function([sents_ids_l, sents_mask_l, sents_ids_r, sents_mask_r, labels], layer_LR.y_pred, allow_input_downcast=True, on_unused_input='ignore')
    ...
    while epoch < n_epochs:
        ...
        for batch_id in train_batch_start: #for each batch
            ...
            if iter%100==0:
                ...
                assert len(test_labels[0]) == len(pred_labels)
                # test_acc=1.0-error_sum/(len(test_batch_start))
                convert_test_labels = []
                for test_label in test_labels[0]:
                    if test_label == 2:
                        convert_test_labels.append(1)
                    else:
                        convert_test_labels.append(0)
                test_f1 = f1_score_binary(convert_test_labels,pred_labels)


                if test_f1 > max_f1:
                    max_f1=test_f1
                print '\t\tcurrent f1:', test_f1,' ; ','\t\tmax_f1: ', max_f1
