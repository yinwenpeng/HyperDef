...
from logistic_sgd_biased import LogisticRegression
...
from load_data import store_word2id, load_SNLI_dataset, write_predictions_to_analysis,load_wordnet_hyper_vs_all_with_words,load_word2vec, load_word2vec_to_init, load_task_hyper_vs_all_with_words
...
BLESS, EVALution, LenciBenotto, Weeds
'''
...
def evaluate_lenet5(learning_rate=0.02, n_epochs=4, L2_weight=1e-5, extra_size=4, emb_size=300, batch_size=100, filter_size=[3,3], maxSentLen=40, hidden_size=[300,300], max_term_len=4, p_mode = 'conc'):
    ...
    test_sents_l_BLESS, test_masks_l_BLESS, test_sents_r_BLESS, test_masks_r_BLESS, test_word1_BLESS,test_word2_BLESS,test_word1_mask_BLESS,test_word2_mask_BLESS,test_labels_BLESS, test_extra_BLESS, word2id = load_task_hyper_vs_all_with_words(BLESS_file,maxSentLen, word2id, wordlen=max_term_len)
    test_sents_l_EVALution, test_masks_l_EVALution, test_sents_r_EVALution, test_masks_r_EVALution, test_word1_EVALution,test_word2_EVALution,test_word1_mask_EVALution,test_word2_mask_EVALution,test_labels_EVALution, test_extra_EVALution, word2id = load_task_hyper_vs_all_with_words(EVALution_file,maxSentLen, word2id, wordlen=max_term_len)
    test_sents_l_LenciBenotto, test_masks_l_LenciBenotto, test_sents_r_LenciBenotto, test_masks_r_LenciBenotto, test_word1_LenciBenotto,test_word2_LenciBenotto,test_word1_mask_LenciBenotto,test_word2_mask_LenciBenotto,test_labels_LenciBenotto, test_extra_LenciBenotto, word2id = load_task_hyper_vs_all_with_words(LenciBenotto_file,maxSentLen, word2id, wordlen=max_term_len)
    test_sents_l_Weeds, test_masks_l_Weeds, test_sents_r_Weeds, test_masks_r_Weeds, test_word1_Weeds,test_word2_Weeds,test_word1_mask_Weeds,test_word2_mask_Weeds,test_labels_Weeds, test_extra_Weeds, word2id = load_task_hyper_vs_all_with_words(Weeds_file,maxSentLen, word2id, wordlen=max_term_len)
    ...
    test_sents_l_BLESS=np.asarray(test_sents_l_BLESS, dtype='int32')
    test_sents_l_EVALution=np.asarray(test_sents_l_EVALution, dtype='int32')
    test_sents_l_LenciBenotto=np.asarray(test_sents_l_LenciBenotto, dtype='int32')
    test_sents_l_Weeds=np.asarray(test_sents_l_Weeds, dtype='int32')
    ...
    test_masks_l_BLESS=np.asarray(test_masks_l_BLESS, dtype=theano.config.floatX)
    test_masks_l_EVALution=np.asarray(test_masks_l_EVALution, dtype=theano.config.floatX)
    test_masks_l_LenciBenotto=np.asarray(test_masks_l_LenciBenotto, dtype=theano.config.floatX)
    test_masks_l_Weeds=np.asarray(test_masks_l_Weeds, dtype=theano.config.floatX)
    ...
    test_sents_r_BLESS=np.asarray(test_sents_r_BLESS, dtype='int32')
    test_sents_r_EVALution=np.asarray(test_sents_r_EVALution, dtype='int32')
    test_sents_r_LenciBenotto=np.asarray(test_sents_r_LenciBenotto, dtype='int32')
    test_sents_r_Weeds=np.asarray(test_sents_r_Weeds, dtype='int32')
        ...
        for batch_id in train_batch_start: #for each batch
            ...
            if iter%100==0:
                ...
                if test_ap > max_ap_test:
                    max_ap_test=test_ap
                    # store_model_to_file('/save/wenpeng/datasets/EVALution/HyperDef_label_4ways_conc_best_para_'+str(max_ap_test), params)
