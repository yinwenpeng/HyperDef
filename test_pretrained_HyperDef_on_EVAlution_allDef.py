...
from load_data import load_word2id, load_task_hyper_vs_all_with_allDefComb, write_predictions_to_analysis,load_wordnet_hyper_vs_all_with_words,load_word2vec, load_word2vec_to_init, load_task_hyper_vs_all_with_words
...
BLESS_file = root_dic+'BLESS.test.all.def.combinations.binary'
EVALution_file = root_dic+'EVALution.test.all.def.combinations.binary'
LenciBenotto_file = root_dic+'LenciBenotto.test.all.def.combinations.binary'
Weeds_file = root_dic+'Weeds.test.all.def.combinations.binary'
...
def evaluate_lenet5(learning_rate=0.02, n_epochs=4, L2_weight=1e-5, extra_size=4, emb_size=300, batch_size=100, filter_size=[3,3], maxSentLen=40, hidden_size=[300,300], max_term_len=4, p_mode = 'conc'):
    ...
    word2id = load_word2id(root_dic+'EVALution_word2id.pkl')
    test_sents_l, test_masks_l, test_sents_r, test_masks_r, test_word1,test_word2,test_word1_mask,test_word2_mask,test_labels, test_extra, word2id, group_size_list = load_task_hyper_vs_all_with_allDefComb(EVALution_file,maxSentLen, word2id, wordlen=max_term_len)
    ...
    load_model_from_file(root_dic+'EVALution_best_para_0.524324268705', params)

    '''
    current ap: 0.622563524472 ap@100 0.927371413831
    '''
