...
def f1_score_binary(golds, preds):
    assert len(golds) == len(preds)
    golds_array = numpy.asarray(golds)
    preds_array = numpy.asarray(preds)
    overlap = numpy.sum(golds_array*preds_array)
    pos_size =numpy.sum(golds_array)
    pos_pred =numpy.sum(preds_array)
    recall = overlap*1.0/pos_size
    precision = overlap*1.0/pos_pred
...
def apk(actual, predicted, k=10):
    ...
    dict = [(prob, label) for prob, label in izip(predicted, actual)] # a list of tuple
    sorted_tuples=sorted(dict,key=lambda tup: tup[0], reverse = True)
    if len(sorted_tuples) > k:
